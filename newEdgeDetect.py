...
class NewEdgeDetector():
    ...
    def setup(self, sess, reuse=False):
        try:
            with tf.name_scope('new_vgg16') as scope:
            #     if reuse:
            #         scope.reuse_variables()
                self.edge_model = Vgg16(self.cfgs,self.img_place_holder,reuse=reuse)

            from tensorflow.python import pywrap_tensorflow
            reader = pywrap_tensorflow.NewCheckpointReader('/home/yyl/pjs/pycharm-remote-data/HED-BSDS/holy-edge/hed/newvgg16/hed-model-5000')
            var_to_shape_map = reader.get_variable_to_shape_map()
            # saver_edge = tf.train.import_meta_graph('/home/yyl/pjs/pycharm-remote-data/HED-BSDS/holy-edge/hed/newvgg16/hed-model-5000.meta')

            t_vars = tf.global_variables()
            var_edge = [var for var in t_vars if 'new_vgg16' in var.name]
            saver_edge = tf.train.Saver(var_edge)
            saver_edge.restore(sess, '/home/yyl/pjs/pycharm-remote-data/HED-BSDS/holy-edge/hed/newvgg16/hed-model-5000')

        except Exception as err:
            self.io.print_error('Error setting up new VGG-16 model, {}'.format(err))
            self.init = False
